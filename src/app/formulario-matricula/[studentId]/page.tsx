...
export default function FormularioMatricula() {
  ...
  const { loading, student } = useSelector((state: RootState) => state.student)
  const [isModalObservacaoVisible, setIsModalObservacaoVisible] =
    useState(false)
  const [isModalResponsavelVisible, setIsModalResponsavelVisible] =
    useState(false)
  ...
  const handleSubmit = () => {
    form
      .validateFields(['aceiteResponsabilidade', 'autorizacaoImagens'])
      .then(() => {
  ...
  if (loading)
    return (
      <div className='flex min-h-screen items-center justify-center'>
        <div className='w-full max-w-4xl rounded-md border bg-white p-6 shadow-lg'>
    ...
    return (
      <div className='flex min-h-screen items-center justify-center'>
        <div className='w-full max-w-4xl rounded-md border bg-white p-6 shadow-lg'>
          ...
          <center>
            <h2 className='mb-6 text-2xl font-semibold'>
              Os dados para a matrícula já foram preenchidos!
            </h2>
          </center>
        </div>
      </div>
  ...
  return (
    <div className='flex min-h-screen items-center justify-center'>
      <div className='w-full max-w-4xl rounded-md border bg-white p-6 shadow-lg'>
        ...
        >
          {/* Dados do Aluno */}
          <div className='mb-6 rounded-md border bg-white p-8 text-center shadow-sm'>
            <h2 className='mb-6 text-2xl font-semibold'>
              Dados para Matrícula
            </h2>
            {student.name && (
              <h2 className='mb-6 text-2xl font-semibold'>{student.name}</h2>
            )}
          ...
          <div className='mb-6 rounded-md border bg-white p-4 shadow-sm'>
            <div className='mb-4 flex items-center justify-between'>
              ...
              <Button
                type='primary'
                onClick={() => setIsModalResponsavelVisible(true)}
              >
                + Adicionar
              </Button>
            </div>
          ...
          {/* Observações Médicas */}
          <div className='mb-6 rounded-md border bg-white p-4 shadow-sm'>
            <h2 className='mb-4 text-lg font-semibold'>Observações Médicas</h2>
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
              ...
              <Form.Item label='Telefone Hospital' name='telefoneHospital'>
                <InputMask mask='(99) 99999-9999'>
                  {inputProps => (
                    <Input {...inputProps} placeholder='Telefone do Hospital' />
                  )}
                </InputMask>
              </Form.Item>
              ...
              <Form.Item label='Telefone Médico' name='telefoneMedico'>
                <InputMask mask='(99) 99999-9999'>
                  {inputProps => (
                    <Input {...inputProps} placeholder='Telefone do Médico' />
                  )}
                </InputMask>
              </Form.Item>
              ...
              <Form.Item
                label='Medicamentos em Caso de Febre'
                name='medicamentosFebre'
              >
                <Input.TextArea placeholder='Descreva os medicamentos' />
              </Form.Item>
              <Form.Item
                label='Medicamentos em Caso de Vômito'
                name='medicamentosVomito'
              >
                <Input.TextArea placeholder='Descreva os medicamentos' />
              </Form.Item>
          ...
          <div className='mb-6 rounded-md border bg-white p-4 shadow-sm'>
            <div className='mb-4 flex items-center justify-between'>
              ...
              <Button
                type='primary'
                onClick={() => setIsModalObservacaoVisible(true)}
              >
                + Adicionar
              </Button>
            </div>
            <Table
              columns={columnsObservacoes}
          ...
          >
            <Checkbox>
              Assumo inteira responsabilidade pelas informações e pelo
              pagamento.
            </Checkbox>
          </Form.Item>
          <Form.Item
          ...
          >
            <Checkbox>
              Autorizo que fotos e filmagens que incluam meu/minha filho(a)
              sejam feitas e utilizadas pela equipe da escola para fins
              pedagógicos, para publicação no site/da escola/da turma, e para
              fins de divulgação nas redes sociais. Estou ciente de que as
              imagens serão usadas apenas para fins pedagógicos e não
              comerciais, resguardadas as limitações legais e jurídicas.
            </Checkbox>
          </Form.Item>
